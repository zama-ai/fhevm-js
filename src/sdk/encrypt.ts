      ...
      return {
        ...
        async _verify(ciphertext: Uint8Array) {
          ...
          try {
            ...
            if (!response.ok) {
              throw new Error(
                `Httpz-relayer didn't response correctly. Bad status ${response.statusText
            ...
            throw new Error(`Httpz-relayer: ${url} didn't response correctly.`, {
              cause: e,
            });
          }
        ...
        async encrypt() {
          ...
          console.log(
            `Encrypting and proving in ${Math.round((Date.now() - start) / 100) / 10
            }s`,
          );
